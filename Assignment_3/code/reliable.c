...
{
	print_pkt(pkt,"received a packet",ntohs(pkt->len));
    ...
    // print_pkt(pkt,"received an ack",pkt->len);
	// print_pkt(&p,"received an ack",ntohs(p.len));
		struct ack_packet *pkt1 = (struct ack_packet *) pkt;
	if (r->client.num_pkt_in_fly > 0) {
		// handle packet
		if (find_and_ack_pkt(r,pkt1) == 0) {
			fprintf(stderr,"[handle_ack_pkt]: error ackno num %d, last_seqno_acked: %d, num_pkt_in_fly: %d\n", \
					pkt1->ackno, r->client.last_seqno_acked, r->client.num_pkt_in_fly);
			exit(1);
		}
		// no pkt fly and
	...
	if (pkt->seqno < min_expected_seqno) {
		// print_pkt(pkt,"sending ack",pkt->len);
...
rel_read (rel_t *r)
{
	if (r->client.state == SEND_WINDOW_NOT_FULL) {
      ...
      // print_pkt(pkt,"sent packet size",pkt_len);
			hton_packet(pkt);
...
// void handle_data_pkt(rel_t *r, packet_t *pkt) {
// 	// print_pkt(pkt,"reading the packet",pkt->len);
...
void send_ack_pkt(rel_t *r, uint32_t ackno) {
	...
	print_pkt(ack_pkt,"sending ack",ack_pkt->len);
	free(ack_pkt);
}
